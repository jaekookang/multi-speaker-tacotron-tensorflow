...
def download_news_video_and_content(
    ...
    original_text_path = os.path.join(
        asset_dir, "original-{}.txt".format(news_id))
    ...
    assert len(article_contents) == 1, \
        "# of <article_contents> of {} should be 1: {}".format(
            news_id, response.text)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(news_id_path):
        while True:
            ...
            print(" [*] Download page {}: {}/{}".format(page_idx,
                                                        len(tmp_ids), len(news_ids)))
