            ...
            if self.num_speakers > 1:
                if hp.speaker_embedding_size != 1:
                    speaker_embed_table = tf.get_variable(
                        'speaker_embedding',
                        [self.num_speakers,
                         hp.speaker_embedding_size], dtype=tf.float32,
                        initializer=tf.truncated_normal_initializer(stddev=0.5))
                ...
                if hp.model_type == 'deepvoice':
                    ...
                    raise Exception(
                        " [!] Unkown multi-speaker model type: {}".format(hp.model_type))
            else:
                speaker_embed = None
                before_highway = None
                encoder_rnn_init_state = None
    ...
    def add_loss(self):
        ...
        with tf.variable_scope('loss') as scope:
            ...
            if hp.prioritize_loss:
                ...
                upper_priority_freq = int(
                    5000 / (hp.sample_rate * 0.5) * hp.num_freq)
                lower_priority_freq = int(
                    165 / (hp.sample_rate * 0.5) * hp.num_freq)
